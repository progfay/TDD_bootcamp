...
describe("下端点と上端点を持つ整数の閉区間クラス", () => {
  ...
  describe("hasメソッドは、指定した整数を含むかどうかを判定できる", () => {
    describe("整数閉区間に対して引数が範囲内であれば、 true を返す", () => {
      it("下端点 3 , 上端点 8 の整数閉区間に対して引数が 5 のとき、 true を返す", () => {
        const closedRange = new ClosedRange(3, 8);
        const actual = closedRange.has(5);
        assert.equal(true, actual);
      });
    });

    describe("整数閉区間に対して引数が範囲外であれば、 false を返す", () => {
      it("下端点 3 , 上端点 8 の整数閉区間に対して引数が 2 のとき、 false を返す", () => {
        const closedRange = new ClosedRange(3, 8);
        const actual = closedRange.has(2);
        assert.equal(false, actual);
      });

      it("下端点 3 , 上端点 8 の整数閉区間に対して引数が 9 のとき、 false を返す", () => {
        const closedRange = new ClosedRange(3, 8);
